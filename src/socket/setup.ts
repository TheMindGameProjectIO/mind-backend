...
io.use(async (socket, next) => {
    ...
    if (!socket.handshake.auth.token) {
        logger.error(
            `socket connection ${socket.id} is rejected due to no token`
        );
    ...
    try {
        ...
        if (!_id) {
            logger.error(
                `socket connection ${socket.id} is rejected due to invalid token (no _id)`
            );
        ...
        if (!userEntity) {
            ...
            logger.warn(
                `socket connection ${socket.id} is processed as anonymous`
            );
        } else {
            logger.info(
                `socket connection ${socket.id} is processed as authenticated with nickname:${userEntity.nickname}`
            );
        }
